...
import org.example.activity.StartJobActivity;
...
public class ServiceServer {
    ...
    private final StartJobActivity someActivity;

    private static final int PORT = 10_000;
    private static final int BACKLOG = 2;

    @Inject
    public ServiceServer(final StartJobActivity someActivity, final ObjectMapper objectMapper) {
    ...
    public void start() throws IOException {
        ...
        server.createContext("/startJob", this::handleStartJob);
        server.setExecutor(Executors.newFixedThreadPool(10));
        server.start();
        log.info("Capture server has started");
    }

    private void handleStartJob(final HttpExchange httpExchange) {
        try {
            setRequestIdInThreadContext();
            if ("POST".equals(httpExchange.getRequestMethod())) {
                final InputStream inputStream = httpExchange.getRequestBody();
                final StartJobRequest startJobRequest = objectMapper.readValue(inputStream, StartJobRequest.class);
                log.info("Received request with request object [{}]", startJobRequest);
                final JobStatusResponse response = someActivity.startJob(startJobRequest);
